...
$.formValidator = 
{
	...
	sustainType : function(id,setting)
	{
		...
		switch(setting.validatetype)
		{
			...
			case "CompareValidator":
				...
				return false;
			case "AjaxValidator":
				if (stype == "password" || stype == "text" || stype == "textarea" || stype == "file" || stype == "select-one"){
					return true;
				}else{
					return false;
				}
	...
	initConfig : function(controlOptions)
	{
		...
		{
			debug:false,
			tipstyle:"both", // both(image + text)、image、text
			errorfocus:false, // focus the first error input
			validatorgroup : "1",
			alertmessage:false,
			validobjectids:"",
	...
	setTipState : function(tipid,showclass,showmsg)
	{
		...
		}else
		{
			...
			if(tip.attr("tipstyle") == "image"){ // 只显示图标
...
$.fn.formValidator = function( msgOptions) 
{
	...
	return this.each(function()
	{
		...
		//初始化显示信息
		if(initConfig.tipstyle == "image"){ // 只显示图标
			$("#" + tip).attr("tipstyle","image").css("padding-left", "16px").css("background-color","transparent").html("&nbsp");
		}else if(initConfig.tipstyle == "text"){ // 只显示文本
			$("#" + tip).attr("tipstyle","text").css("padding-left", "0px").css("margin-left", "5px").css("background-image", "none");
		}
		if(!initConfig.alertmessage){
			...
			jqobj.bind(setting.triggerevent, function(){   
				...
				if(returnObj.ajax >= 0) 
				{
					if(this.validoldvalue!=$(this).val())
					{
						...
						$.formValidator.ajaxValid(returnObj);
					}else if(!settings[returnObj.ajax].isvalid){
						if(settings[returnObj.ajax].lastValid==""){
							$.formValidator.setTipState(tip,"onLoad",settings[returnObj.ajax].onwait);
							$.formValidator.ajaxValid(returnObj);
						}else{
							$.formValidator.setTipState(tip,"onError",settings[returnObj.ajax].onerror);
						}
					}else{
						$.formValidator.setTipState(tip,"onSuccess",setting.oncorrect);
					}
				}
				else
				{
		...
		else if (srcTag == "SELECT")
		{
			...
			//选择项目后触发
			jqobj.bind(stype=="select-one"?setting.triggerevent:"blur" , function()
			{
				var returnObj = $.formValidator.oneIsValid(this.id,1);	
				if(returnObj==null){return;}
