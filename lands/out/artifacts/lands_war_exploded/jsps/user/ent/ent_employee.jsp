...
<form action="<c:url value='/UserServlet'/>" method="get">
    ...
    <!--隐藏字段-->
    <input type="hidden" name="method" value="searchEnt"/>
    ...
    <table border="1" width="90%" cellspacing="0" id="">
        ...
        <c:forEach items="${pageBean.beanList}" var="entList">
            <tr>
                ...
                <td>${entList.groups.g_name}</td>
                <td>${entList.enterprise.enname}</td>
                <td>${entList.add_date}</td>
                <td>${entList.creator}</td>
                <td><a href="<c:url value='/UserServlet?method=findEntByU_id&u_id=${entList.u_id}'/> ">查看</a>
                    <a href="<c:url value='/UserServlet?method=loadModifyEnt&u_id=${entList.u_id}'/> ">修改</a>
                    <a href="<c:url value='/UserServlet?method=deleteEnt&u_id=${entList.u_id}'/> ">删除</a>
                </td>
            </tr>
        </c:forEach>
        </tbody>
